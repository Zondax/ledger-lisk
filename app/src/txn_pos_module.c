...
static parser_error_t parse_stake(parser_context_t *ctx, tx_command_pos_stake_t *transfer) {
    uint64_t stakeSize = 0;
    uint64_t addressSize = 0;
    int64_t amount = 0;
    int64_t absoluteAmount = 0;

    transfer->start = ctx->buffer + ctx->offset;
    while (ctx->offset < ctx->bufferLen) {
        // Get stake size
        GET_KEY_AND_VARUINT(ctx, stakeSize);
        transfer->stakeSize[transfer->n_stake] = stakeSize;

        // Get address size
        GET_KEY_AND_VARUINT(ctx, addressSize);
        if (addressSize != ADDRESS_HASH_LENGTH) {
            return parser_unexpected_value;
        }
        ...
        // Get amount
        CHECK_ERROR(_readUnsignedVarint(ctx, (uint64_t*) &amount))
        CHECK_ERROR(_readSignedVarint(ctx, &amount))
        absoluteAmount = (amount < 0 ? -amount : amount);

        if (absoluteAmount == 0 || (absoluteAmount % BASE_STAKE_AMOUNT != 0)) {
             return parser_unexpected_value;
        }
        transfer->amounts[transfer->n_stake] = amount;
        transfer->n_stake++;
...
static parser_error_t parse_change_commission(parser_context_t *ctx, tx_command_pos_change_commissions_t *transfer) {
...
parser_error_t parse_pos_module(parser_context_t *ctx, parser_tx_t *tx_obj) {
    ...
    switch (tx_obj->command_id) {
        ...
        case TX_COMMAND_ID_CHANGE_COMMISSION:
            CHECK_ERROR(parse_change_commission(ctx, &tx_obj->tx_command._pos_change_commissions))
            break;
        default:
